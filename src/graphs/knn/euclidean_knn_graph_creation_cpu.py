...
def _euclidean_knn_worker(patient_item, k, graph_dir):
    """
...
def euclidean_knn_graph_creation_cpu(patient_dict, k=8, num_workers=8):
    """
    Iterates through all the patients, creates a graph for each patient using
    KNN on CLS features, and saves each graph to disk using multiple CPU cores.
    """
    ...
    print(f"Starting parallel knn graph creation (CPU) for {len(patient_dict)} patients with k={k}...")

    # --- 2. Process Patients in Parallel ---
    ctx = mp.get_context('spawn')
    worker_fn = partial(_euclidean_knn_worker, k=k, graph_dir=graph_dir)
