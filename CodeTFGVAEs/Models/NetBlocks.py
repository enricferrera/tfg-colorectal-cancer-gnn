...
def linear_block(n_inputs_loc, hidden_loc, 
    ...
    for i in np.arange(len(hidden_loc)-1):
        ...
        elif(activ_config[i]=='softmax'):
             BlockArchitecture.append(('soft'+str(i+1),nn.Softmax()))
...
class _CNNLayer(nn.Module):
    def __init__(
        ...
        if dim==2:
            ...
            norm1 = nn.BatchNorm1d(n_neurons)
            conv1 = nn.Conv1d(num_input_features, n_neurons,kernel_size=kernel_sze, 
                                   stride=stride,padding=(int((kernel_sze-1)/2))
...
class _UnCNNLayer(nn.Module):
    def __init__(
        ...
        if dim==2:
            ...
            norm1 = nn.BatchNorm1d(n_neurons)
            conv1 = nn.ConvTranspose1d(num_input_features, n_neurons,kernel_size=kernel_sze, 
                                   stride=stride,padding=(int((kernel_sze-1)/2))
                                   )
        relu1 = nn.ReLU(inplace=True)
    ...
    def forward(self, x: Tensor):
        ...
        if  self.stride>1:
            sze_enc=x.shape[-1]
            output_size=tuple(np.repeat(sze_enc*2, self.dim))
            x=self.cnn_layer[0](x,output_size=output_size)
